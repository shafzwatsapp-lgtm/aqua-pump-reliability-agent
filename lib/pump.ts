...
export function analyse(scenario:Scenario,finding:Finding='none'){
  ...
  const evidence:Evidence[]=[
    ...
    {id:'VB-02',source:'Vibration · calculated',title:'Frequency signature',detail:quality?'Spectral features are withheld because the source channel is missing or sensor mounting is reported loose. Re-acquire a valid signal before interpretation.':`Hann-window FFT: 2× (50 Hz) peak ${wave.twoX.toFixed(2)} mm/s peak. Energy in 300–900 Hz represents ${(ratio*100).toFixed(0)}% of the 1–1,000 Hz spectral sum.`,quality:quality?'Invalid source · derived features withheld':'1 Hz resolution · relative spectral energy'},
