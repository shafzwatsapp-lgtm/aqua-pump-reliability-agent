...
export async function POST(request:Request){try{
 ...
 const answer=await runEvidenceAgent({result,question:body.question,history:(body.history||[]).slice(-8),complete:async(messages,round)=>{
  const response=await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${settings.OPENROUTER_API_KEY}`,'Content-Type':'application/json','X-OpenRouter-Title':'AQUA Pump Reliability'},body:JSON.stringify({model:settings.OPENROUTER_MODEL||'google/gemini-3-flash-preview',provider:{require_parameters:true},max_tokens:2200,reasoning:{effort:'low',exclude:true},temperature:.2,messages,tools:agentTools,tool_choice:round===0?'required':'auto'}),signal});
  if(!response.ok){console.warn('AQUA provider HTTP status',response.status);throw Error('PROVIDER');}const data=await response.json() as {choices?:{message?:ModelReply;finish_reason?:string}[]};if(!data.choices?.[0]?.message)throw Error('MODEL');return {...data.choices[0].message,finish_reason:data.choices[0].finish_reason};
