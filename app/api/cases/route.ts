...
export async function GET(request:Request){try{const owner=await identity();const scenario=new URL(request.url).searchParams.get('scenario');if(!scenarios.some(x=>x.id===scenario))throw Error('INPUT');const data=await database().prepare('SELECT id,kind,body,created FROM case_events WHERE owner=? AND scenario=? ORDER BY created DESC,rowid DESC LIMIT 250').bind(owner,scenario).all();return Response.json({events:data.results.reverse().map(x=>({...x,body:JSON.parse(String(x.body))}))});}catch(e){return failure(e);}}
export async function POST(request:Request){try{
 ...
 if(!scenarios.some(x=>x.id===input.scenario)||!findings.some(x=>x.id===input.finding))throw Error('INPUT');
