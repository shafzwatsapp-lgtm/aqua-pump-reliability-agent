...
Teams verification on September 12, 2026: the local AQUA API rejected signed-out requests, reserved delivery IDs and stale tasks, accepted one approved-task notification and suppressed a repeat. The matching synthetic card was visibly received in the account owner's private Workflows bot chat at 16:07 UAE. The corrected workflow uses Chat with Flow bot because Microsoft rejected the special You self-chat as a non-chat thread. The same application code and private server configuration were deployed successfully; an additional production-origin send was not performed.
